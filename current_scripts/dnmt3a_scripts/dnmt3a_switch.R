...
### This code runs the FLT3-NPM1-DNMT3A subgroup from the Palma paper, and uses a switch() function to knock in/out genes based on the patient mutation profile
...
patient_profile_df <- s7_combined_df
...
scorePersonal <- function(mutation_profile) { 
  ...
  for (i in mutation_profile){
    switch(i, 
           ...
           "GSK3B"={time.series$GSK3B <- 0},   #look up info
           "NPM1"={time.series$NPM1<- 0},    #look up info
           "ARF"={time.series$ARF <- 0},	   #look up info
           "HOXA9"={time.series$HOXA9 <- 1},   #look up info
           "FBXW7"={time.series$FBXW7 <- 0},   #look up info
           "ERK"={time.series$ERK <- 1},    #look up info
           "CDKN2A"={time.series$CDKN2A <- 0}, #look up info
           "STAT5A"={time.series$STAT5A <- 1},  #look up info
           "SOX4"={time.series$SOX4 <- 1},	 #look up info
           "CCND1"={time.series$CCND1 <- 1},	 #look up info
           "MEIS1"={time.series$MEIS1 <- 1},   #look up info
           "MYC"={time.series$MYC <- 1},	
           "ETV6"={time.series$ETV6 <- 0},
           "TP53"={time.series$TP53 <- 0},	  #look up info  
