...
n.obs <- 40000 # number of observation aka steps 
...
scorePersonal <- function(mutation_profile) { 
  ...
  for (col in 2:n.obs){
    ...
    if (any(grepl("RAD21", mutation_profile))) {time.series[20,col] <- 0}  	
    ...
    if (any(grepl("BRAF", mutation_profile))) {time.series[41,col] <- 0}    
    if (any(grepl("CBFB", mutation_profile))) {time.series[42,col] <- 0}   
    if (any(grepl("CDK6", mutation_profile))) {time.series[43,col] <- 0}  
    if (any(grepl("MECOM", mutation_profile))) {time.series[44,col] <- 1}	   		  
    if (any(grepl("CBFbeta_MYH11", mutation_profile))) {time.series[45,col] <- 1}	 #check the lettering! 
    if (any(grepl("NPM1", mutation_profile))) {time.series[46,col] <- 0}	  
    if (any(grepl("IDH1", mutation_profile))) {time.series[47,col] <- 1}   
    if (any(grepl("IDH2", mutation_profile))) {time.series[48,col] <- 1}    
    if (any(grepl("ERK1/2", mutation_profile))) {time.series[49,col] <- 0}   #check the lettering!	
    ...
    if (any(grepl("AP1", mutation_profile))) {time.series[57,col] <- 0}  
    ...
    if (any(grepl("MTOR", mutation_profile))) {time.series[62,col] <- 0}    #check the value
    ...
    if (any(grepl("SRSF2", mutation_profile))) {time.series[68,col] <- 0}   #check the value
    ...
    if (any(grepl("MDM2", mutation_profile))) {time.series[73,col] <- 0}	 #check the value  
